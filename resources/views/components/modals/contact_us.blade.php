<div x-cloak x-show="showContactUsModal" @keyup.escape.window="$refs.closeContactUsModal.click()" class="absolute top-[50%] left-1/2 right-1/2 w-[95%] sm:w-[525px] h-[450px] -translate-y-1/2 -translate-x-1/2 bg-stone-300 rounded text-slate-700 p-4 border-2 border-indigo-500 z-10 flex flex-col">
    <div class="flex-1">
        <h2 class="text-xl text-center">Send us a message and we will make it a priority to get back to you.</h2>
        <form method="post" action="{{route('contact_us_post_url')}}" class="mt-5" x-ref="contact_us_form">
            @csrf
            ...
            <div class="form-group mb-1">
                <div class="checkbox">
                    <label class="ml-1" for="contact_confirm">
                        <input type="checkbox" name="contact_confirm" class="h-[20px] w-[20px] align-sub cursor-pointer"> Send me a confirmation
                    </label>
                </div>
            </div>
        </form>
    ...
    <div class="flex justify-center gap-[4px]">
        ...
        <button class="app-btn app-btn-primary" @click="$refs.contact_us_form.submit()">Send</button>
    </div>
</div>